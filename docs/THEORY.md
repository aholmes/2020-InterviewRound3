...
One issue that may arrise is with huge data sets, but in reality, how many customers will purchase the quadrillion+ products that would be necessary to significantly slow down a BK Tree?
...
Generally, I would start with considering what is actually possible or likely. As I started in the previous question, it is unlikely customers will purchase an unbelievably number of products. This means the use of an efficient text searching data structure and algorithm, like a BK Tree, should never become a bottle neck. Hopefully the bottle neck is I/O (network, disk, etc).
